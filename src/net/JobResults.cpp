...
#if defined(HashburstMiner_FEATURE_OPENCL) || defined(HashburstMiner_FEATURE_CUDA)
...
{
public:
    HashburstMiner_DISABLE_COPY_MOVE_DEFAULT(JobResultsPrivate)
...
void HashburstMiner::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void HashburstMiner::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void HashburstMiner::JobResults::stop()
{
    assert(handler != nullptr);
...
void HashburstMiner::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void HashburstMiner::JobResults::submit(const Job& job, uint32_t nonce, const uint8_t* result, const uint8_t* miner_signature)
{
    submit(JobResult(job, nonce, result, nullptr, nullptr, miner_signature));
}


void HashburstMiner::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(HashburstMiner_FEATURE_OPENCL) || defined(HashburstMiner_FEATURE_CUDA)
void HashburstMiner::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
