...
static const char *kDonateHost = "donate.v2.HashburstMiner.com";
#ifdef HashburstMiner_FEATURE_TLS
static const char *kDonateHostTls = "donate.ssl.HashburstMiner.com";
#endif

} // namespace HashburstMiner


HashburstMiner::DonateStrategy::DonateStrategy(Controller *controller, IStrategyListener *listener) :
...
#   if defined HashburstMiner_ALGO_KAWPOW || defined HashburstMiner_ALGO_GHOSTRIDER
...
HashburstMiner::DonateStrategy::~DonateStrategy()
{
    delete m_timer;
    delete m_strategy;
...
void HashburstMiner::DonateStrategy::update(IClient *client, const Job &job)
{
...
int64_t HashburstMiner::DonateStrategy::submit(const JobResult &result)
{
    return m_proxy ? m_proxy->submit(result) : m_strategy->submit(result);
}


void HashburstMiner::DonateStrategy::connect()
{
    m_proxy = createProxy();
    if (m_proxy) {
        m_proxy->connect();
...
void HashburstMiner::DonateStrategy::setAlgo(const HashburstMiner::Algorithm &algo)
{
    m_algorithm = algo;
...
void HashburstMiner::DonateStrategy::setProxy(const ProxyUrl &proxy)
{
    m_strategy->setProxy(proxy);
}


void HashburstMiner::DonateStrategy::stop()
{
    m_timer->stop();
    m_strategy->stop();
}
...
void HashburstMiner::DonateStrategy::tick(uint64_t now)
{
    m_now = now;
...
void HashburstMiner::DonateStrategy::onActive(IStrategy *, IClient *client)
{
    if (isActive()) {
        return;
    }
...
void HashburstMiner::DonateStrategy::onPause(IStrategy *)
{
}


void HashburstMiner::DonateStrategy::onClose(IClient *, int failures)
{
...
void HashburstMiner::DonateStrategy::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
void HashburstMiner::DonateStrategy::onLogin(IStrategy *, IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
    setParams(doc, params);
}


void HashburstMiner::DonateStrategy::onLoginSuccess(IClient *client)
{
    if (isActive()) {
        return;
    }
...
void HashburstMiner::DonateStrategy::onVerifyAlgorithm(const IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void HashburstMiner::DonateStrategy::onVerifyAlgorithm(IStrategy *, const  IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void HashburstMiner::DonateStrategy::onTimer(const Timer *)
{
    setState(isActive() ? STATE_WAIT : STATE_CONNECT);
}


HashburstMiner::IClient *HashburstMiner::DonateStrategy::createProxy()
{
...
void HashburstMiner::DonateStrategy::idle(double min, double max)
{
    m_timer->start(random(m_idleTime, min, max), 0);
}


void HashburstMiner::DonateStrategy::setJob(IClient *client, const Job &job, const rapidjson::Value &params)
{
    if (isActive()) {
...
void HashburstMiner::DonateStrategy::setParams(rapidjson::Document &doc, rapidjson::Value &params)
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void HashburstMiner::DonateStrategy::setResult(IClient *client, const SubmitResult &result, const char *error)
{
    m_listener->onResultAccepted(this, client, result, error);
}


void HashburstMiner::DonateStrategy::setState(State state)
{
