...
{
public:
    HashburstMiner_DISABLE_COPY_MOVE_DEFAULT(MinerPrivate)
...
HashburstMiner::Miner::Miner(Controller *controller)
...
HashburstMiner::Miner::~Miner()
{
    delete d_ptr;
}


bool HashburstMiner::Miner::isEnabled() const
{
    return d_ptr->enabled;
}


bool HashburstMiner::Miner::isEnabled(const Algorithm &algorithm) const
{
    return std::find(d_ptr->algorithms.begin(), d_ptr->algorithms.end(), algorithm) != d_ptr->algorithms.end();
}


const HashburstMiner::Algorithms &HashburstMiner::Miner::algorithms() const
{
    return d_ptr->algorithms;
}


const std::vector<HashburstMiner::IBackend *> &HashburstMiner::Miner::backends() const
{
    return d_ptr->backends;
}


HashburstMiner::Job HashburstMiner::Miner::job() const
{
...
void HashburstMiner::Miner::execCommand(char command)
{
    switch (command) {
    case 'h':
    case 'H':
...
void HashburstMiner::Miner::pause()
{
    d_ptr->active = false;
...
void HashburstMiner::Miner::setEnabled(bool enabled)
{
...
void HashburstMiner::Miner::setJob(const Job &job, bool donate)
{
...
void HashburstMiner::Miner::stop()
{
    Nonce::stop();
...
void HashburstMiner::Miner::onConfigChanged(Config *config, Config *previousConfig)
{
    d_ptr->rebuild();
...
void HashburstMiner::Miner::onTimer(const Timer *)
{
...
#ifdef HashburstMiner_FEATURE_API
void HashburstMiner::Miner::onRequest(IApiRequest &request)
{
...
#ifdef HashburstMiner_ALGO_RANDOMX
void HashburstMiner::Miner::onDatasetReady()
{
    if (!Rx::isReady(job())) {
        return;
    }
