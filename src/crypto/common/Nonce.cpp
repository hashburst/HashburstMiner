...
bool HashburstMiner::Nonce::next(uint8_t index, uint32_t *nonce, uint32_t reserveCount, uint64_t mask)
{
...
void HashburstMiner::Nonce::stop()
{
    pause(false);
...
void HashburstMiner::Nonce::touch()
{
    for (auto &i : m_sequence) {
        i++;
    }
