...
HashburstMiner::MemoryPool::MemoryPool(size_t size, bool hugePages, uint32_t node)
{
    if (!size) {
        return;
    }
...
HashburstMiner::MemoryPool::~MemoryPool()
{
    delete m_memory;
}


bool HashburstMiner::MemoryPool::isHugePages(uint32_t) const
{
    return m_memory && m_memory->isHugePages();
}


uint8_t *HashburstMiner::MemoryPool::get(size_t size, uint32_t)
{
...
void HashburstMiner::MemoryPool::release(uint32_t)
{
    assert(m_refs > 0);
