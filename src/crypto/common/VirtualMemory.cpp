...
HashburstMiner::VirtualMemory::VirtualMemory(size_t size, bool hugePages, bool oneGbPages, bool usePool, uint32_t node, size_t alignSize) :
...
HashburstMiner::VirtualMemory::~VirtualMemory()
{
    if (!m_scratchpad) {
        return;
    }
...
HashburstMiner::HugePagesInfo HashburstMiner::VirtualMemory::hugePages() const
{
    return { this };
}


#ifndef HashburstMiner_FEATURE_HWLOC
uint32_t HashburstMiner::VirtualMemory::bindToNUMANode(int64_t)
{
    return 0;
}
#endif
...
void HashburstMiner::VirtualMemory::destroy()
{
    delete pool;
}


void HashburstMiner::VirtualMemory::init(size_t poolSize, size_t hugePageSize)
{
    if (!pool) {
        osInit(hugePageSize);
    }
