...
HashburstMiner::RxCache::RxCache(bool hugePages, uint32_t nodeId)
{
...
HashburstMiner::RxCache::RxCache(uint8_t *memory)
{
    create(memory);
}


HashburstMiner::RxCache::~RxCache()
{
...
bool HashburstMiner::RxCache::init(const Buffer &seed)
{
    if (m_seed == seed) {
        return false;
    }
...
HashburstMiner::HugePagesInfo HashburstMiner::RxCache::hugePages() const
{
    return m_memory ? m_memory->hugePages() : HugePagesInfo();
}


void HashburstMiner::RxCache::create(uint8_t *memory)
{
    if (!memory) {
        return;
    }
