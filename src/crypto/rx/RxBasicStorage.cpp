...
{
public:
    HashburstMiner_DISABLE_COPY_MOVE(RxBasicStoragePrivate)
...
HashburstMiner::RxBasicStorage::RxBasicStorage() :
...
bool HashburstMiner::RxBasicStorage::isAllocated() const
{
    return d_ptr->dataset() && d_ptr->dataset()->cache() && d_ptr->dataset()->cache()->get();
}


HashburstMiner::HugePagesInfo HashburstMiner::RxBasicStorage::hugePages() const
{
    if (!d_ptr->dataset()) {
        return {};
    }
...
HashburstMiner::RxDataset *HashburstMiner::RxBasicStorage::dataset(const Job &job, uint32_t) const
{
...
void HashburstMiner::RxBasicStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode mode, int priority)
{
    d_ptr->setSeed(seed);
