...
HashburstMiner::HugePagesInfo HashburstMiner::Rx::hugePages()
{
    return d_ptr->queue.hugePages();
}


HashburstMiner::RxDataset *HashburstMiner::Rx::dataset(const Job &job, uint32_t nodeId)
{
    return d_ptr->queue.dataset(job, nodeId);
}


void HashburstMiner::Rx::destroy()
{
#   ifdef HashburstMiner_FEATURE_MSR
    RxMsr::destroy();
#   endif
...
void HashburstMiner::Rx::init(IRxListener *listener)
{
...
#if defined(HashburstMiner_FEATURE_AVX2)
...
template<typename T>
bool HashburstMiner::Rx::init(const T &seed, const RxConfig &config, const CpuConfig &cpu)
{
...
#       if defined(HashburstMiner_FEATURE_SSE4_1)
...
template<typename T>
bool HashburstMiner::Rx::isReady(const T &seed)
{
