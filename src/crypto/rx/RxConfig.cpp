...
#ifdef HashburstMiner_FEATURE_HWLOC
...
bool HashburstMiner::RxConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
rapidjson::Value HashburstMiner::RxConfig::toJSON(rapidjson::Document &doc) const
{
...
#ifdef HashburstMiner_FEATURE_HWLOC
std::vector<uint32_t> HashburstMiner::RxConfig::nodeset() const
{
    if (!m_nodeset.empty()) {
        return m_nodeset;
    }
...
const char *HashburstMiner::RxConfig::modeName() const
{
    return modeNames[m_mode];
}


uint32_t HashburstMiner::RxConfig::threads(uint32_t limit) const
{
    if (m_threads > 0) {
        return m_threads;
    }
...
#ifdef HashburstMiner_FEATURE_MSR
const char *HashburstMiner::RxConfig::msrPresetName() const
{
    return modNames[msrMod()];
}


const HashburstMiner::MsrItems &HashburstMiner::RxConfig::msrPreset() const
{
...
uint32_t HashburstMiner::RxConfig::msrMod() const
{
    if (!wrmsr()) {
...
void HashburstMiner::RxConfig::readMSR(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
HashburstMiner::RxConfig::Mode HashburstMiner::RxConfig::readMode(const rapidjson::Value &value)
{
    if (value.IsUint()) {
