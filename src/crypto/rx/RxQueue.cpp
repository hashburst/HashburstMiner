...
HashburstMiner::RxQueue::RxQueue(IRxListener *listener) :
    m_listener(listener)
{
...
HashburstMiner::RxQueue::~RxQueue()
{
...
HashburstMiner::RxDataset *HashburstMiner::RxQueue::dataset(const Job &job, uint32_t nodeId)
{
...
HashburstMiner::HugePagesInfo HashburstMiner::RxQueue::hugePages()
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
template<typename T>
bool HashburstMiner::RxQueue::isReady(const T &seed)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
void HashburstMiner::RxQueue::enqueue(const RxSeed &seed, const std::vector<uint32_t> &nodeset, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode mode, int priority)
{
    std::unique_lock<std::mutex> lock(m_mutex);
...
template<typename T>
bool HashburstMiner::RxQueue::isReadyUnsafe(const T &seed) const
{
    return m_storage != nullptr && m_storage->isAllocated() && m_state == STATE_IDLE && m_seed == seed;
}


void HashburstMiner::RxQueue::backgroundInit()
{
...
void HashburstMiner::RxQueue::onReady()
{
    std::unique_lock<std::mutex> lock(m_mutex);
