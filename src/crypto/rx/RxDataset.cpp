...
HashburstMiner::RxDataset::RxDataset(bool hugePages, bool oneGbPages, bool cache, RxConfig::Mode mode, uint32_t node) :
    m_mode(mode),
    m_node(node)
{
...
HashburstMiner::RxDataset::RxDataset(RxCache *cache) :
    m_node(0),
    m_cache(cache)
{
}
...
HashburstMiner::RxDataset::~RxDataset()
{
...
bool HashburstMiner::RxDataset::init(const Buffer &seed, uint32_t numThreads, int priority)
{
...
bool HashburstMiner::RxDataset::isHugePages() const
{
    return m_memory && m_memory->isHugePages();
}


bool HashburstMiner::RxDataset::isOneGbPages() const
{
    return m_memory && m_memory->isOneGbPages();
}


HashburstMiner::HugePagesInfo HashburstMiner::RxDataset::hugePages(bool cache) const
{
...
size_t HashburstMiner::RxDataset::size(bool cache) const
{
    size_t size = 0;
...
uint8_t *HashburstMiner::RxDataset::tryAllocateScrathpad()
{
...
void *HashburstMiner::RxDataset::raw() const
{
    return m_dataset ? randomx_get_dataset_memory(m_dataset) : nullptr;
}


void HashburstMiner::RxDataset::setRaw(const void *raw)
{
    if (!m_dataset) {
        return;
    }
...
void HashburstMiner::RxDataset::allocate(bool hugePages, bool oneGbPages)
{
