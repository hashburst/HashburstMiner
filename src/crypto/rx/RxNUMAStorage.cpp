...
{
public:
    HashburstMiner_DISABLE_COPY_MOVE_DEFAULT(RxNUMAStoragePrivate)
...
HashburstMiner::RxNUMAStorage::RxNUMAStorage(const std::vector<uint32_t> &nodeset) :
    d_ptr(new RxNUMAStoragePrivate(nodeset))
{
}


HashburstMiner::RxNUMAStorage::~RxNUMAStorage()
{
    delete d_ptr;
}


bool HashburstMiner::RxNUMAStorage::isAllocated() const
{
    return d_ptr->isAllocated();
}


HashburstMiner::HugePagesInfo HashburstMiner::RxNUMAStorage::hugePages() const
{
...
HashburstMiner::RxDataset *HashburstMiner::RxNUMAStorage::dataset(const Job &job, uint32_t nodeId) const
{
...
void HashburstMiner::RxNUMAStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode, int priority)
{
    d_ptr->setSeed(seed);
