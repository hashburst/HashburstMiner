...
static inline void add_random_math(uint8_t* &p, const V4_Instruction* code, int code_size, const void_func* instructions, const void_func* instructions_mov, bool is_64_bit, HashburstMiner::Assembly::Id ASM)
{
    ...
    for (int i = 0;; ++i) {
        ...
        if ((ASM == HashburstMiner::Assembly::BULLDOZER) && (inst.opcode == MUL) && !is_64_bit) {
...
void v4_compile_code(const V4_Instruction* code, int code_size, void* machine_code, HashburstMiner::Assembly ASM)
{
    ...
    *(int*)(p - 4) = static_cast<int>((((const uint8_t*)CryptonightR_template_mainloop) - ((const uint8_t*)CryptonightR_template_part1)) - (p - p0));
    ...
    HashburstMiner::VirtualMemory::flushInstructionCache(machine_code, p - p0);
}
