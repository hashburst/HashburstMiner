...
HashburstMiner::Algorithm::Algorithm(const rapidjson::Value &value) :
    m_id(parse(value.GetString()))
{
}


HashburstMiner::Algorithm::Algorithm(uint32_t id) :
    m_id(kAlgorithmNames.count(id) ? static_cast<Id>(id) : INVALID)
{
}


const char *HashburstMiner::Algorithm::name() const
{
    if (!isValid()) {
        return kINVALID;
    }
...
rapidjson::Value HashburstMiner::Algorithm::toJSON() const
{
...
rapidjson::Value HashburstMiner::Algorithm::toJSON(rapidjson::Document &) const
{
    return toJSON();
}


HashburstMiner::Algorithm::Id HashburstMiner::Algorithm::parse(const char *name)
{
...
size_t HashburstMiner::Algorithm::count()
{
    return kAlgorithmNames.size();
}


std::vector<HashburstMiner::Algorithm> HashburstMiner::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
