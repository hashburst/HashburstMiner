...
HashburstMiner::HttpsContext::HttpsContext(TlsContext *tls, const std::weak_ptr<IHttpListener> &listener) :
...
HashburstMiner::HttpsContext::~HttpsContext() = default;


void HashburstMiner::HttpsContext::append(char *data, size_t size)
{
    if (m_mode == TLS_AUTO) {
...
bool HashburstMiner::HttpsContext::write(BIO *bio)
{
...
void HashburstMiner::HttpsContext::parse(char *data, size_t size)
{
...
void HashburstMiner::HttpsContext::shutdown()
{
    close();
}


void HashburstMiner::HttpsContext::write(std::string &&data, bool close)
{
    m_close = close;
