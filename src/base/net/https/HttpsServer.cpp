...
HashburstMiner::HttpsServer::HttpsServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


HashburstMiner::HttpsServer::~HttpsServer()
{
...
bool HashburstMiner::HttpsServer::setTls(const TlsConfig &config)
{
...
void HashburstMiner::HttpsServer::onConnection(uv_stream_t *stream, uint16_t)
{
...
void HashburstMiner::HttpsServer::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
