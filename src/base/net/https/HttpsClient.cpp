...
HashburstMiner::HttpsClient::HttpsClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
HashburstMiner::HttpsClient::~HttpsClient()
{
    if (m_ctx) {
        SSL_CTX_free(m_ctx);
    }
...
const char *HashburstMiner::HttpsClient::tlsFingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *HashburstMiner::HttpsClient::tlsVersion() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void HashburstMiner::HttpsClient::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
void HashburstMiner::HttpsClient::read(const char *data, size_t size)
{
...
void HashburstMiner::HttpsClient::write(std::string &&data, bool close)
{
...
bool HashburstMiner::HttpsClient::verify(X509 *cert)
{
    if (cert == nullptr) {
        return false;
    }
...
bool HashburstMiner::HttpsClient::verifyFingerprint(X509 *cert)
{
...
void HashburstMiner::HttpsClient::flush(bool close)
{
