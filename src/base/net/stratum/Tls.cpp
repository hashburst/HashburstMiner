...
HashburstMiner::Client::Tls::Tls(Client *client) :
    m_client(client)
{
...
bool HashburstMiner::Client::Tls::handshake(const char* servername)
{
    m_ssl = SSL_new(m_ctx);
...
bool HashburstMiner::Client::Tls::send(const char *data, size_t size)
{
...
const char *HashburstMiner::Client::Tls::fingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *HashburstMiner::Client::Tls::version() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void HashburstMiner::Client::Tls::read(const char *data, size_t size)
{
...
bool HashburstMiner::Client::Tls::send()
{
    return m_client->send(m_write);
}


bool HashburstMiner::Client::Tls::verify(X509 *cert)
{
    if (cert == nullptr) {
...
bool HashburstMiner::Client::Tls::verifyFingerprint(X509 *cert)
{
