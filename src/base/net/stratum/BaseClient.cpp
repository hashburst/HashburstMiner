...
HashburstMiner::BaseClient::BaseClient(int id, IClientListener *listener) :
    m_listener(listener),
    m_id(id)
{
}
...
void HashburstMiner::BaseClient::setPool(const Pool &pool)
{
    if (!pool.isValid()) {
        return;
    }
...
bool HashburstMiner::BaseClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == 1) {
        return false;
    }
...
bool HashburstMiner::BaseClient::handleSubmitResponse(int64_t id, const char *error)
{
