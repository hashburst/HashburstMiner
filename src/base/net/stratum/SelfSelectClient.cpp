...
HashburstMiner::SelfSelectClient::SelfSelectClient(int id, const char *agent, IClientListener *listener, bool submitToOrigin) :
...
HashburstMiner::SelfSelectClient::~SelfSelectClient()
{
    delete m_client;
}


int64_t HashburstMiner::SelfSelectClient::submit(const JobResult &result)
{
    if (m_submitToOrigin) {
...
void HashburstMiner::SelfSelectClient::tick(uint64_t now)
{
    m_client->tick(now);
...
void HashburstMiner::SelfSelectClient::onJobReceived(IClient *, const Job &job, const rapidjson::Value &)
{
    m_job = job;
...
void HashburstMiner::SelfSelectClient::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
bool HashburstMiner::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
void HashburstMiner::SelfSelectClient::getBlockTemplate()
{
    setState(WaitState);
...
void HashburstMiner::SelfSelectClient::retry()
{
    setState(RetryState);
}


void HashburstMiner::SelfSelectClient::setState(State state)
{
    if (m_state == state) {
        return;
    }
...
void HashburstMiner::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    using namespace rapidjson;
    Document doc(kObjectType);
    auto &allocator = doc.GetAllocator();
...
void HashburstMiner::SelfSelectClient::submitOriginDaemon(const JobResult& result)
{
...
void HashburstMiner::SelfSelectClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
