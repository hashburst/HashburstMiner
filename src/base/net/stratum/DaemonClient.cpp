...
HashburstMiner::DaemonClient::DaemonClient(int id, IClientListener *listener) :
...
HashburstMiner::DaemonClient::~DaemonClient()
{
    delete m_timer;
    delete m_ZMQSocket;
}
...
void HashburstMiner::DaemonClient::deleteLater()
{
...
bool HashburstMiner::DaemonClient::disconnect()
{
...
bool HashburstMiner::DaemonClient::isTLS() const
{
#   ifdef HashburstMiner_FEATURE_TLS
    return m_pool.isTLS();
#   else
...
int64_t HashburstMiner::DaemonClient::submit(const JobResult &result)
{
...
void HashburstMiner::DaemonClient::connect()
{
...
void HashburstMiner::DaemonClient::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void HashburstMiner::DaemonClient::setPool(const Pool &pool)
{
...
void HashburstMiner::DaemonClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
...
void HashburstMiner::DaemonClient::onTimer(const Timer *)
{
    if (m_pool.zmq_port() >= 0) {
        m_prevHash = nullptr;
...
void HashburstMiner::DaemonClient::onResolved(const DnsRecords &records, int status, const char* error)
{
    m_dns.reset();
...
bool HashburstMiner::DaemonClient::isOutdated(uint64_t height, const char *hash) const
{
    return m_job.height() != height || m_prevHash != hash || Chrono::steadyMSecs() >= m_jobSteadyMs + m_pool.jobTimeout();
}


bool HashburstMiner::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
...
bool HashburstMiner::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
int64_t HashburstMiner::DaemonClient::getBlockTemplate()
{
    using namespace rapidjson;
    Document doc(kObjectType);
...
int64_t HashburstMiner::DaemonClient::rpcSend(const rapidjson::Document &doc, const std::map<std::string, std::string> &headers)
{
...
void HashburstMiner::DaemonClient::retry()
{
    m_failures++;
...
void HashburstMiner::DaemonClient::send(const char *path)
{
...
void HashburstMiner::DaemonClient::setState(SocketState state)
{
    if (m_state == state) {
        return;
    }
...
void HashburstMiner::DaemonClient::onZMQConnect(uv_connect_t* req, int status)
{
...
void HashburstMiner::DaemonClient::onZMQRead(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
...
void HashburstMiner::DaemonClient::onZMQClose(uv_handle_t* handle)
{
...
void HashburstMiner::DaemonClient::onZMQShutdown(uv_handle_t* handle)
{
    DaemonClient* client = getClient(handle->data);
    if (client) {
#       ifdef APP_DEBUG
...
void HashburstMiner::DaemonClient::ZMQConnected()
{
#   ifdef APP_DEBUG
...
bool HashburstMiner::DaemonClient::ZMQWrite(const char* data, size_t size)
{
...
void HashburstMiner::DaemonClient::ZMQRead(ssize_t nread, const uv_buf_t* buf)
{
    if (nread <= 0) {
...
void HashburstMiner::DaemonClient::ZMQParse()
{
#   ifdef APP_DEBUG
    std::vector<char> msg;
#   endif
...
#   endif
    ...
    // Clear previous hash and check daemon height to guarantee that HashburstMiner will call get_block_template RPC later
...
bool HashburstMiner::DaemonClient::ZMQClose(bool shutdown)
{
    if ((m_ZMQConnectionState == ZMQ_NOT_CONNECTED) || (m_ZMQConnectionState == ZMQ_DISCONNECTING)) {
        if (shutdown) {
