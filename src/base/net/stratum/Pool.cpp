...
HashburstMiner::Pool::Pool(const char *url) :
...
HashburstMiner::Pool::Pool(const char *host, uint16_t port, const char *user, const char *password, const char* spendSecretKey, int keepAlive, bool nicehash, bool tls, Mode mode) :
...
HashburstMiner::Pool::Pool(const rapidjson::Value &object) :
    m_flags(1 << FLAG_ENABLED),
    m_pollInterval(kDefaultPollInterval),
    m_jobTimeout(kDefaultJobTimeout),
...
#ifdef HashburstMiner_FEATURE_BENCHMARK
HashburstMiner::Pool::Pool(const std::shared_ptr<BenchConfig> &benchmark) :
...
HashburstMiner::BenchConfig *HashburstMiner::Pool::benchmark() const
{
...
uint32_t HashburstMiner::Pool::benchSize() const
{
...
bool HashburstMiner::Pool::isEnabled() const
{
#   ifndef HashburstMiner_FEATURE_TLS
    if (isTLS()) {
        return false;
    }
#   endif

#   ifndef HashburstMiner_FEATURE_HTTP
...
bool HashburstMiner::Pool::isEqual(const Pool &other) const
{
...
HashburstMiner::IClient *HashburstMiner::Pool::createClient(int id, IClientListener *listener) const
{
...
rapidjson::Value HashburstMiner::Pool::toJSON(rapidjson::Document &doc) const
{
...
std::string HashburstMiner::Pool::printableName() const
{
...
#ifdef APP_DEBUG
void HashburstMiner::Pool::print() const
{
...
void HashburstMiner::Pool::setKeepAlive(const rapidjson::Value &value)
{
    if (value.IsInt()) {
