 ...
 * Copyright 2016-2024 HashburstMiner       <https://github.com/hashburst/HashburstMiner>, <pegoraro.gabriele@gmail.com>
 *
...
HashburstMiner::Job::Job(bool nicehash, const Algorithm &algorithm, const String &clientId) :
...
bool HashburstMiner::Job::isEqual(const Job &other) const
{
    return m_id == other.m_id && m_clientId == other.m_clientId && isEqualBlob(other) && m_target == other.m_target;
}


bool HashburstMiner::Job::isEqualBlob(const Job &other) const
{
    return (m_size == other.m_size) && (memcmp(m_blob, other.m_blob, m_size) == 0);
}


bool HashburstMiner::Job::setBlob(const char *blob)
{
    if (!blob) {
        return false;
    }
...
bool HashburstMiner::Job::setSeedHash(const char *hash)
{
...
bool HashburstMiner::Job::setTarget(const char *target)
{
...
size_t HashburstMiner::Job::nonceOffset() const
{
...
void HashburstMiner::Job::setDiff(uint64_t diff)
{
    m_diff   = diff;
...
void HashburstMiner::Job::setSigKey(const char *sig_key)
{
...
uint32_t HashburstMiner::Job::getNumTransactions() const
{
...
void HashburstMiner::Job::copy(const Job &other)
{
...
void HashburstMiner::Job::move(Job &&other)
{
    m_algorithm  = other.m_algorithm;
    m_nicehash   = other.m_nicehash;
    m_size       = other.m_size;
...
void HashburstMiner::Job::setSpendSecretKey(const uint8_t *key)
{
...
void HashburstMiner::Job::setMinerTx(const uint8_t *begin, const uint8_t *end, size_t minerTxEphPubKeyOffset, size_t minerTxPubKeyOffset, size_t minerTxExtraNonceOffset, size_t minerTxExtraNonceSize, const Buffer &minerTxMerkleTreeBranch, bool hasViewTag)
{
...
void HashburstMiner::Job::setViewTagInMinerTx(uint8_t view_tag)
{
    memcpy(m_minerTxPrefix.data() + m_minerTxEphPubKeyOffset + 32, &view_tag, 1);
}


void HashburstMiner::Job::setExtraNonceInMinerTx(uint32_t extra_nonce)
{
    memcpy(m_minerTxPrefix.data() + m_minerTxExtraNonceOffset, &extra_nonce, std::min(m_minerTxExtraNonceSize, sizeof(uint32_t)));
}


void HashburstMiner::Job::generateSignatureData(String &signatureData, uint8_t& view_tag) const
{
...
void HashburstMiner::Job::generateHashingBlob(String &blob) const
{
...
void HashburstMiner::Job::generateMinerSignature(const uint8_t* blob, size_t size, uint8_t* out_sig) const
{
    ...
    HashburstMiner::keccak(tmp, static_cast<int>(size), prefix_hash, sizeof(prefix_hash));
    HashburstMiner::generate_signature(prefix_hash, m_ephPublicKey, m_ephSecretKey, out_sig);
}
