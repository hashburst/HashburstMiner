...
HashburstMiner::Client::Client(int id, const char *agent, IClientListener *listener) :
...
HashburstMiner::Client::~Client()
{
    delete m_socket;
}


bool HashburstMiner::Client::disconnect()
{
    m_keepAlive = 0;
    m_expire    = 0;
    m_failures  = -1;
...
bool HashburstMiner::Client::isTLS() const
{
#   ifdef HashburstMiner_FEATURE_TLS
...
const char *HashburstMiner::Client::tlsFingerprint() const
{
#   ifdef HashburstMiner_FEATURE_TLS
...
const char *HashburstMiner::Client::tlsVersion() const
{
#   ifdef HashburstMiner_FEATURE_TLS
    if (isTLS()) {
...
int64_t HashburstMiner::Client::send(const rapidjson::Value &obj, Callback callback)
{
...
int64_t HashburstMiner::Client::send(const rapidjson::Value &obj)
{
...
int64_t HashburstMiner::Client::submit(const JobResult &result)
{
#   ifndef HashburstMiner_PROXY_PROJECT
...
void HashburstMiner::Client::connect()
{
...
void HashburstMiner::Client::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void HashburstMiner::Client::deleteLater()
{
    if (!m_listener) {
        return;
    }
...
void HashburstMiner::Client::tick(uint64_t now)
{
...
void HashburstMiner::Client::onResolved(const DnsRecords &records, int status, const char *error)
{
    m_dns.reset();
...
bool HashburstMiner::Client::close()
{
...
bool HashburstMiner::Client::parseJob(const rapidjson::Value &params, int *code)
{
    if (!params.IsObject()) {
        *code = 2;
        return false;
...
#   ifdef HashburstMiner_FEATURE_HTTP
...
#   endif
    ...
    if (m_jobs == 0) { // https://github.com/hashburst/HashburstMiner/HashburstMiner/issues/459
        return false;
    }
...
bool HashburstMiner::Client::send(BIO *bio)
{
#   ifdef HashburstMiner_FEATURE_TLS
    uv_buf_t buf;
...
bool HashburstMiner::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
...
bool HashburstMiner::Client::write(const uv_buf_t &buf)
{
...
int HashburstMiner::Client::resolve(const String &host)
{
...
int64_t HashburstMiner::Client::send(size_t size)
{
...
void HashburstMiner::Client::connect(const sockaddr *addr)
{
...
void HashburstMiner::Client::handshake()
{
    if (m_socks5) {
...
bool HashburstMiner::Client::parseLogin(const rapidjson::Value &result, int *code)
{
...
void HashburstMiner::Client::login()
{
    using namespace rapidjson;
    m_results.clear();
...
void HashburstMiner::Client::onClose()
{
    delete m_socket;
...
void HashburstMiner::Client::parse(char *line, size_t len)
{
    startTimeout();
...
void HashburstMiner::Client::parseExtensions(const rapidjson::Value &result)
{
    m_extensions.reset();
...
void HashburstMiner::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void HashburstMiner::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void HashburstMiner::Client::ping()
{
...
void HashburstMiner::Client::read(ssize_t nread, const uv_buf_t *buf)
{
...
void HashburstMiner::Client::reconnect()
{
    if (!m_listener) {
        m_storage.remove(m_key);
...
void HashburstMiner::Client::setState(SocketState state)
{
...
void HashburstMiner::Client::startTimeout()
{
    m_expire = 0;
...
bool HashburstMiner::Client::isCriticalError(const char *message)
{
    if (!message) {
        return false;
    }
...
void HashburstMiner::Client::onClose(uv_handle_t *handle)
{
...
void HashburstMiner::Client::onConnect(uv_connect_t *req, int status)
{
...
void HashburstMiner::Client::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
