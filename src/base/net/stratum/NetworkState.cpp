...
HashburstMiner::NetworkState::NetworkState(IStrategyListener *listener) : StrategyProxy(listener)
{
}


#ifdef HashburstMiner_FEATURE_API
rapidjson::Value HashburstMiner::NetworkState::getConnection(rapidjson::Document &doc, int version) const
{
...
rapidjson::Value HashburstMiner::NetworkState::getResults(rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void HashburstMiner::NetworkState::printConnection() const
{
    if (!m_active) {
...
void HashburstMiner::NetworkState::printResults() const
{
    if (!m_hashes) {
...
const char *HashburstMiner::NetworkState::scaleDiff(uint64_t &diff)
{
...
std::string HashburstMiner::NetworkState::humanDiff(uint64_t diff)
{
...
void HashburstMiner::NetworkState::onActive(IStrategy *strategy, IClient *client)
{
...
void HashburstMiner::NetworkState::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &params)
{
...
void HashburstMiner::NetworkState::onPause(IStrategy *strategy)
{
...
void HashburstMiner::NetworkState::onResultAccepted(IStrategy *strategy, IClient *client, const SubmitResult &result, const char *error)
{
    add(result, error);
...
uint32_t HashburstMiner::NetworkState::latency() const
{
...
uint64_t HashburstMiner::NetworkState::avgTime() const
{
    if (m_latency.empty()) {
        return 0;
    }
...
uint64_t HashburstMiner::NetworkState::connectionTime() const
{
    return m_active ? ((Chrono::steadyMSecs() - m_connectionTime)) : 0;
}


void HashburstMiner::NetworkState::add(const SubmitResult &result, const char *error)
{
    if (error) {
        m_rejected++;
        return;
...
void HashburstMiner::NetworkState::stop()
{
    m_active      = false;
    m_diff        = 0;
    m_ip          = nullptr;
