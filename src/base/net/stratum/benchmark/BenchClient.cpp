...
HashburstMiner::BenchClient::BenchClient(const std::shared_ptr<BenchConfig> &benchmark, IClientListener* listener) :
    m_listener(listener),
...
#   ifdef HashburstMiner_ALGO_GHOSTRIDER
...
HashburstMiner::BenchClient::~BenchClient()
{
    BenchState::destroy();
}


const char *HashburstMiner::BenchClient::tag() const
{
    return Tags::bench();
}


void HashburstMiner::BenchClient::connect()
{
#   ifdef HashburstMiner_FEATURE_HTTP
...
void HashburstMiner::BenchClient::setPool(const Pool &pool)
{
    m_pool = pool;
}


void HashburstMiner::BenchClient::onBenchDone(uint64_t result, uint64_t diff, uint64_t ts)
{
    m_result    = result;
    m_diff      = diff;
    m_doneTime  = ts;
...
void HashburstMiner::BenchClient::onBenchReady(uint64_t ts, uint32_t threads, const IBackend *backend)
{
    m_readyTime = ts;
    m_threads   = threads;
    m_backend   = backend;
...
void HashburstMiner::BenchClient::onHttpData(const HttpData &data)
{
#   ifdef HashburstMiner_FEATURE_HTTP
...
void HashburstMiner::BenchClient::onResolved(const DnsRecords &records, int status, const char *error)
{
#   ifdef HashburstMiner_FEATURE_HTTP
...
bool HashburstMiner::BenchClient::setSeed(const char *seed)
{
    if (!seed) {
        return false;
    }
...
uint64_t HashburstMiner::BenchClient::referenceHash() const
{
...
void HashburstMiner::BenchClient::printExit() const
{
    LOG_INFO("%s " WHITE_BOLD("press ") MAGENTA_BOLD("Ctrl+C") WHITE_BOLD(" to exit"), tag());
}


void HashburstMiner::BenchClient::start()
{
...
#ifdef HashburstMiner_FEATURE_HTTP
void HashburstMiner::BenchClient::onCreateReply(const rapidjson::Value &value)
{
...
void HashburstMiner::BenchClient::onDoneReply(const rapidjson::Value &)
{
    LOG_NOTICE("%s " WHITE_BOLD("benchmark submitted ") CYAN_BOLD("https://HashburstMiner.com/benchmark/%s"), tag(), m_job.id().data());
    printExit();
}


void HashburstMiner::BenchClient::onGetReply(const rapidjson::Value &value)
{
...
void HashburstMiner::BenchClient::resolve()
{
    m_dns = Dns::resolve(BenchConfig::kApiHost, this);
}


void HashburstMiner::BenchClient::send(Request request)
{
...
void HashburstMiner::BenchClient::setError(const char *message, const char *label)
{
...
void HashburstMiner::BenchClient::update(const rapidjson::Value &body)
{
