...
HashburstMiner::HttpResponse::HttpResponse(uint64_t id, int statusCode) :
    m_id(id),
...
bool HashburstMiner::HttpResponse::isAlive() const
{
...
void HashburstMiner::HttpResponse::end(const char *data, size_t size)
{
    if (!isAlive()) {
        return;
    }
