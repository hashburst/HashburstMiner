...
HashburstMiner::HttpClient::HttpClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
bool HashburstMiner::HttpClient::connect()
{
...
void HashburstMiner::HttpClient::onResolved(const DnsRecords &records, int status, const char *error)
{
    this->status = status;
    m_dns.reset();
...
void HashburstMiner::HttpClient::onTimer(const Timer *)
{
    close(UV_ETIMEDOUT);
}


void HashburstMiner::HttpClient::handshake()
{
...
void HashburstMiner::HttpClient::read(const char *data, size_t size)
{
    if (!parse(data, size)) {
        close(UV_EPROTO);
    }
...
void HashburstMiner::HttpClient::onConnect(uv_connect_t *req, int status)
{
