...
{
public:
    HashburstMiner_DISABLE_COPY_MOVE_DEFAULT(HttpWriteBaton)
...
HashburstMiner::HttpContext::HttpContext(int parser_type, const std::weak_ptr<IHttpListener> &listener) :
    HttpData(SEQUENCE++),
...
HashburstMiner::HttpContext::~HttpContext()
{
    delete m_tcp;
    delete m_parser;
}
...
void HashburstMiner::HttpContext::write(std::string &&data, bool close)
{
...
bool HashburstMiner::HttpContext::isRequest() const
{
    return m_parser->type == HTTP_REQUEST;
}


bool HashburstMiner::HttpContext::parse(const char *data, size_t size)
{
    if (size == 0) {
        return true;
    }
...
std::string HashburstMiner::HttpContext::ip() const
{
    char ip[46]           = {};
...
uint64_t HashburstMiner::HttpContext::elapsed() const
{
    return Chrono::steadyMSecs() - m_timestamp;
}


void HashburstMiner::HttpContext::close(int status)
{
    if (!get(id())) {
        return;
    }
...
HashburstMiner::HttpContext *HashburstMiner::HttpContext::get(uint64_t id)
{
...
void HashburstMiner::HttpContext::closeAll()
{
    for (auto &kv : storage) {
...
int HashburstMiner::HttpContext::onHeaderField(llhttp_t *parser, const char *at, size_t length)
{
...
int HashburstMiner::HttpContext::onHeaderValue(llhttp_t *parser, const char *at, size_t length)
{
    auto ctx = static_cast<HttpContext*>(parser->data);
...
void HashburstMiner::HttpContext::attach(llhttp_settings_t *settings)
{
...
void HashburstMiner::HttpContext::setHeader()
{
