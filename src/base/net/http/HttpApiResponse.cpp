...
HashburstMiner::HttpApiResponse::HttpApiResponse(uint64_t id) :
    HttpResponse(id),
...
HashburstMiner::HttpApiResponse::HttpApiResponse(uint64_t id, int status) :
    HttpResponse(id),
    m_doc(rapidjson::kObjectType)
{
    setStatus(status);
...
void HashburstMiner::HttpApiResponse::end()
{
