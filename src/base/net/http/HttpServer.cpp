...
HashburstMiner::HttpServer::HttpServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


HashburstMiner::HttpServer::~HttpServer()
{
    HttpContext::closeAll();
}


void HashburstMiner::HttpServer::onConnection(uv_stream_t *stream, uint16_t)
{
