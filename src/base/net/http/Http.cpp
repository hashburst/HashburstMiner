...
HashburstMiner::Http::Http() :
    m_host(kLocalhost)
{
}


bool HashburstMiner::Http::isEqual(const Http &other) const
{
...
rapidjson::Value HashburstMiner::Http::toJSON(rapidjson::Document &doc) const
{
...
void HashburstMiner::Http::load(const rapidjson::Value &http)
{
    if (!http.IsObject()) {
        return;
    }
...
void HashburstMiner::Http::setPort(int port)
{
