...
bool HashburstMiner::HttpData::isJSON() const
{
...
const char *HashburstMiner::HttpData::methodName() const
{
    return llhttp_method_name(static_cast<llhttp_method>(method));
}


rapidjson::Document HashburstMiner::HttpData::json() const
{
    if (status < 0) {
...
const char *HashburstMiner::HttpData::statusName(int status)
{
    if (status < 0) {
