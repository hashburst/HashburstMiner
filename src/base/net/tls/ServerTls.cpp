...
HashburstMiner::ServerTls::ServerTls(SSL_CTX *ctx) :
    m_ctx(ctx)
{
}


HashburstMiner::ServerTls::~ServerTls()
{
    if (m_ssl) {
        SSL_free(m_ssl);
    }
...
bool HashburstMiner::ServerTls::isHTTP(const char *data, size_t size)
{
    assert(size > 0);
...
bool HashburstMiner::ServerTls::isTLS(const char *data, size_t size)
{
    assert(size > 0);
...
bool HashburstMiner::ServerTls::send(const char *data, size_t size)
{
...
void HashburstMiner::ServerTls::read(const char *data, size_t size)
{
    if (!m_ssl) {
        m_ssl = SSL_new(m_ctx);
...
void HashburstMiner::ServerTls::read()
{
