...
HashburstMiner::TlsContext::~TlsContext()
{
    SSL_CTX_free(m_ctx);
}


HashburstMiner::TlsContext *HashburstMiner::TlsContext::create(const TlsConfig &config)
{
    if (!config.isEnabled()) {
        return nullptr;
    }
...
bool HashburstMiner::TlsContext::load(const TlsConfig &config)
{
...
bool HashburstMiner::TlsContext::setCiphers(const char *ciphers)
{
...
bool HashburstMiner::TlsContext::setCipherSuites(const char *ciphersuites)
{
...
bool HashburstMiner::TlsContext::setDH(const char *dhparam)
{
#   if OPENSSL_VERSION_NUMBER < 0x30000000L || defined(LIBRESSL_VERSION_NUMBER)
    DH *dh = nullptr;
...
void HashburstMiner::TlsContext::setProtocols(uint32_t protocols)
{
    if (protocols == 0) {
        return;
    }
