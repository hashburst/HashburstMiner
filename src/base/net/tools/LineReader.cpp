...
HashburstMiner::LineReader::~LineReader()
{
    NetBuffer::release(m_buf);
}


void HashburstMiner::LineReader::parse(char *data, size_t size)
{
...
void HashburstMiner::LineReader::reset()
{
    if (m_buf) {
        NetBuffer::release(m_buf);
        m_buf = nullptr;
...
void HashburstMiner::LineReader::add(const char *data, size_t size)
{
    if (size + m_pos > HashburstMiner_NET_BUFFER_CHUNK_SIZE) {
...
void HashburstMiner::LineReader::getline(char *data, size_t size)
{
    char *end        = nullptr;
    char *start      = data;
