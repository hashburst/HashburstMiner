...
static const HashburstMiner::String kLocalHost("127.0.0.1");


HashburstMiner::TcpServer::TcpServer(const String &host, uint16_t port, ITcpServerListener *listener) :
...
HashburstMiner::TcpServer::~TcpServer()
{
    Handle::close(m_tcp);
}


int HashburstMiner::TcpServer::bind()
{
    if (!m_version) {
...
void HashburstMiner::TcpServer::create(uv_stream_t *stream, int status)
{
    if (status < 0) {
        return;
    }
...
void HashburstMiner::TcpServer::onConnection(uv_stream_t *stream, int status)
{
