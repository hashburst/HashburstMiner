...
#ifdef HashburstMiner_FEATURE_EMBEDDED_CONFIG
...
{
public:
    HashburstMiner_DISABLE_COPY_MOVE_DEFAULT(BasePrivate)
...
private:
    inline static Config *load(Process *process)
    {
        ...
        chain.addFile(Process::location(Process::HomeLocation, ".config" HashburstMiner_DIR_SEPARATOR APP_ID ".json"));
        if (read(chain, config)) {
            return config.release();
        }
...
HashburstMiner::Base::Base(Process *process)
...
HashburstMiner::Base::~Base()
{
    delete d_ptr;
}


bool HashburstMiner::Base::isReady() const
{
    return d_ptr->config != nullptr;
}


int HashburstMiner::Base::init()
{
#   ifdef HashburstMiner_FEATURE_API
...
void HashburstMiner::Base::start()
{
#   ifdef HashburstMiner_FEATURE_API
    api()->start();
#   endif
...
HashburstMiner::Api *HashburstMiner::Base::api() const
{
...
bool HashburstMiner::Base::isBackground() const
{
    return d_ptr->config && d_ptr->config->isBackground();
}


bool HashburstMiner::Base::reload(const rapidjson::Value &json)
{
...
HashburstMiner::Config *HashburstMiner::Base::config() const
{
...
void HashburstMiner::Base::addListener(IBaseListener *listener)
{
    d_ptr->listeners.push_back(listener);
}


void HashburstMiner::Base::onFileChanged(const String &fileName)
{
...
#ifdef HashburstMiner_FEATURE_API
void HashburstMiner::Base::onRequest(IApiRequest &request)
{
