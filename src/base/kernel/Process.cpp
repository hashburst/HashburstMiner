...
#       endif
    ...
    if (location == Process::ExeLocation) {
        ...
        const auto pos  = path.rfind(*HashburstMiner_DIR_SEPARATOR);
...
HashburstMiner::Process::Process(int argc, char **argv) :
...
#   ifdef HashburstMiner_SHARED_DATADIR
    if (dataDir.empty()) {
        dataDir = fmt::format("{}" HashburstMiner_DIR_SEPARATOR ".HashburstMiner" HashburstMiner_DIR_SEPARATOR, location(HomeLocation));
        MKDIR(dataDir);
...
int HashburstMiner::Process::ppid()
{
...
HashburstMiner::String HashburstMiner::Process::exepath()
{
    size_t size = sizeof(pathBuf);
...
HashburstMiner::String HashburstMiner::Process::location(Location location, const char *fileName)
{
    ...
    return fmt::format("{}" HashburstMiner_DIR_SEPARATOR "{}", path, fileName).c_str();
}
