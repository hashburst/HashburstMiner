...
char *HashburstMiner::Platform::createUserAgent()
{
...
bool HashburstMiner::Platform::setThreadAffinity(uint64_t cpu_id)
{
    return true;
}
...
#endif // __DragonFly__
#endif // HashburstMiner_FEATURE_HWLOC


void HashburstMiner::Platform::setProcessPriority(int)
{
}


void HashburstMiner::Platform::setThreadPriority(int priority)
{
    if (priority == -1) {
        return;
    }
...
bool HashburstMiner::Platform::isOnBatteryPower()
{
    for (int i = 0; i <= 1; ++i) {
        char buf[64];
...
uint64_t HashburstMiner::Platform::idleTime()
{
