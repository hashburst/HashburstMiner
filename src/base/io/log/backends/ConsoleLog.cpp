...
HashburstMiner::ConsoleLog::ConsoleLog(const Title &title)
{
    if (!isSupported()) {
        Log::setColors(false);
        return;
...
HashburstMiner::ConsoleLog::~ConsoleLog()
{
    Handle::close(m_tty);
}


void HashburstMiner::ConsoleLog::print(uint64_t, int, const char *line, size_t, size_t size, bool colors)
{
...
bool HashburstMiner::ConsoleLog::isSupported()
{
...
#ifdef HashburstMiner_OS_WIN
bool HashburstMiner::ConsoleLog::isWritable() const
{
