...
HashburstMiner::FileLogWriter::FileLogWriter()
{
    init();
}

HashburstMiner::FileLogWriter::FileLogWriter(const char* fileName)
{
    init();
    open(fileName);
}
...
void HashburstMiner::FileLogWriter::init()
{
...
bool HashburstMiner::FileLogWriter::open(const char *fileName)
{
...
bool HashburstMiner::FileLogWriter::write(const char *data, size_t size)
{
    if (!isOpen()) {
        return false;
    }
...
bool HashburstMiner::FileLogWriter::writeLine(const char *data, size_t size)
{
    if (!isOpen()) {
        return false;
    }
...
void HashburstMiner::FileLogWriter::flush()
{
    uv_mutex_lock(&m_buffersLock);
