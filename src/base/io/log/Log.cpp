...
{
public:
    HashburstMiner_DISABLE_COPY_MOVE(LogPrivate)
...
void HashburstMiner::Log::add(ILogBackend *backend)
{
    assert(d != nullptr);
...
void HashburstMiner::Log::destroy()
{
    delete d;
    d = nullptr;
}
...
void HashburstMiner::Log::init()
{
    d = new LogPrivate();
}


void HashburstMiner::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void HashburstMiner::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
