...
#ifdef SIGUSR1
static const int signums[HashburstMiner::Signals::kSignalsCount] = { SIGHUP, SIGINT, SIGTERM, SIGUSR1 };
#else
static const int signums[HashburstMiner::Signals::kSignalsCount] = { SIGHUP, SIGINT, SIGTERM };
#endif


HashburstMiner::Signals::Signals(ISignalListener *listener)
    : m_listener(listener)
{
#   ifndef HashburstMiner_OS_WIN
...
HashburstMiner::Signals::~Signals()
{
...
void HashburstMiner::Signals::onSignal(uv_signal_t *handle, int signum)
{
    switch (signum)
    {
    case SIGHUP:
