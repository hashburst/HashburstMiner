...
HashburstMiner::Console::Console(IConsoleListener *listener)
    : m_listener(listener)
{
    if (!isSupported()) {
        return;
...
HashburstMiner::Console::~Console()
{
    uv_tty_reset_mode();
...
bool HashburstMiner::Console::isSupported()
{
...
void HashburstMiner::Console::onAllocBuffer(uv_handle_t *handle, size_t, uv_buf_t *buf)
{
...
void HashburstMiner::Console::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
    if (nread < 0) {
