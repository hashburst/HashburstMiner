...
bool HashburstMiner::Json::get(const char *fileName, rapidjson::Document &doc)
{
...
bool HashburstMiner::Json::save(const char *fileName, const rapidjson::Document &doc)
{
...
#   ifdef HashburstMiner_JSON_SINGLE_LINE_ARRAY
...
bool HashburstMiner::Json::convertOffset(const char* fileName, size_t offset, size_t& line, size_t& pos, std::vector<std::string>& s)
{
    std::ifstream ifs(fileName, std::ios_base::in | std::ios_base::binary);
    if (!ifs.is_open()) {
        return false;
