...
HashburstMiner::JsonChain::JsonChain() = default;


bool HashburstMiner::JsonChain::add(rapidjson::Document &&doc)
{
...
bool HashburstMiner::JsonChain::addFile(const char *fileName)
{
...
bool HashburstMiner::JsonChain::addRaw(const char *json)
{
    using namespace rapidjson;
    Document doc;
...
void HashburstMiner::JsonChain::dump(const char *fileName)
{
...
bool HashburstMiner::JsonChain::getBool(const char *key, bool defaultValue) const
{
...
const char *HashburstMiner::JsonChain::getString(const char *key, const char *defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &HashburstMiner::JsonChain::getArray(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &HashburstMiner::JsonChain::getObject(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &HashburstMiner::JsonChain::getValue(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
        if (i != it->MemberEnd()) {
...
const rapidjson::Value &HashburstMiner::JsonChain::object() const
{
    assert(false);
...
double HashburstMiner::JsonChain::getDouble(const char *key, double defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
int HashburstMiner::JsonChain::getInt(const char *key, int defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
int64_t HashburstMiner::JsonChain::getInt64(const char *key, int64_t defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
HashburstMiner::String HashburstMiner::JsonChain::getString(const char *key, size_t maxSize) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
        if (i != it->MemberEnd() && i->value.IsString()) {
...
uint64_t HashburstMiner::JsonChain::getUint64(const char *key, uint64_t defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
unsigned HashburstMiner::JsonChain::getUint(const char *key, unsigned defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
