...
cl_program HashburstMiner::OclCache::build(const IOclRunner *runner)
{
...
#       ifdef _WIN32
        fileName = prefix() + "\\HashburstMiner\\.cache\\" + cacheKey(runner) + ".bin";
#       else
...
std::string HashburstMiner::OclCache::cacheKey(const char *deviceKey, const char *options, const char *source)
{
...
std::string HashburstMiner::OclCache::cacheKey(const IOclRunner *runner)
{
    return cacheKey(runner->deviceKey(), runner->buildOptions(), runner->source());
}


void HashburstMiner::OclCache::save(cl_program program, const std::string &fileName)
{
    size_t size = 0;
