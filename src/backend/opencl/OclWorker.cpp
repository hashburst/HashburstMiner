...
HashburstMiner::OclWorker::OclWorker(size_t id, const OclLaunchData &data) :
...
HashburstMiner::OclWorker::~OclWorker()
{
    delete m_runner;
}


void HashburstMiner::OclWorker::jobEarlyNotification(const Job &job)
{
    if (m_runner) {
...
bool HashburstMiner::OclWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t HashburstMiner::OclWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void HashburstMiner::OclWorker::start()
{
...
bool HashburstMiner::OclWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::OPENCL) == 0) {
        return false;
    }
...
void HashburstMiner::OclWorker::storeStats(uint64_t t)
{
    if (!isReady()) {
        return;
    }
