...
bool HashburstMiner::AdlLib::init()
{
    if (!m_initialized) {
        m_ready       = dlopen() && load();
        m_initialized = true;
...
const char *HashburstMiner::AdlLib::lastError() noexcept
{
    return nullptr;
}


void HashburstMiner::AdlLib::close()
{
}


// https://dri.freedesktop.org/docs/drm/gpu/amdgpu.html#gpu-power-thermal-controls-and-monitoring
AdlHealth HashburstMiner::AdlLib::health(const OclDevice &device)
{
...
bool HashburstMiner::AdlLib::dlopen()
{
    struct stat sb;
...
bool HashburstMiner::AdlLib::load()
{
    return true;
}
