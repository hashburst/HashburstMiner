...
HashburstMiner::OclDevice::OclDevice(uint32_t index, cl_device_id id, cl_platform_id platform) :
    m_id(id),
    m_platform(platform),
...
HashburstMiner::String HashburstMiner::OclDevice::printableName() const
{
...
uint32_t HashburstMiner::OclDevice::clock() const
{
    return OclLib::getUint(id(), CL_DEVICE_MAX_CLOCK_FREQUENCY);
}


void HashburstMiner::OclDevice::generate(const Algorithm &algorithm, OclThreads &threads) const
{
...
void HashburstMiner::OclDevice::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
