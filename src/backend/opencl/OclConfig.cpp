...
#ifndef HashburstMiner_OS_APPLE
...
#ifdef HashburstMiner_FEATURE_ADL
...
#ifndef HashburstMiner_OS_APPLE
HashburstMiner::OclConfig::OclConfig() : m_platformVendor(kAMD) {}
#else
HashburstMiner::OclConfig::OclConfig() = default;
#endif


HashburstMiner::OclPlatform HashburstMiner::OclConfig::platform() const
{
...
rapidjson::Value HashburstMiner::OclConfig::toJSON(rapidjson::Document &doc) const
{
...
std::vector<HashburstMiner::OclLaunchData> HashburstMiner::OclConfig::get(const Miner *miner, const Algorithm &algorithm, const OclPlatform &platform, const std::vector<OclDevice> &devices) const
{
...
void HashburstMiner::OclConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void HashburstMiner::OclConfig::generate()
{
    ...
    count += HashburstMiner::generate<Algorithm::CN>(m_threads, devices);
    count += HashburstMiner::generate<Algorithm::CN_LITE>(m_threads, devices);
    count += HashburstMiner::generate<Algorithm::CN_HEAVY>(m_threads, devices);
    count += HashburstMiner::generate<Algorithm::CN_PICO>(m_threads, devices);
    count += HashburstMiner::generate<Algorithm::CN_FEMTO>(m_threads, devices);
    count += HashburstMiner::generate<Algorithm::RANDOM_X>(m_threads, devices);
    count += HashburstMiner::generate<Algorithm::KAWPOW>(m_threads, devices);

    m_shouldSave = count > 0;
}


void HashburstMiner::OclConfig::setDevicesHint(const char *devicesHint)
{
...
#ifndef HashburstMiner_OS_APPLE
void HashburstMiner::OclConfig::setPlatform(const rapidjson::Value &platform)
{
