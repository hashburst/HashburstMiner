...
HashburstMiner::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
bool HashburstMiner::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value HashburstMiner::OclThread::toJSON(rapidjson::Document &doc) const
{
