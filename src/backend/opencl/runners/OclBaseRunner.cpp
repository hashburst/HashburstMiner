...
HashburstMiner::OclBaseRunner::OclBaseRunner(size_t id, const OclLaunchData &data) :
    m_ctx(data.ctx),
...
#   ifdef HashburstMiner_STRICT_OPENCL_CACHE
    m_deviceKey += ":";
...
HashburstMiner::OclBaseRunner::~OclBaseRunner()
{
...
size_t HashburstMiner::OclBaseRunner::bufferSize() const
{
    return align(Job::kMaxBlobSize) + align(sizeof(cl_uint) * 0x100);
}


uint32_t HashburstMiner::OclBaseRunner::deviceIndex() const
{
    return data().thread.index();
}


void HashburstMiner::OclBaseRunner::build()
{
...
void HashburstMiner::OclBaseRunner::init()
{
...
cl_mem HashburstMiner::OclBaseRunner::createSubBuffer(cl_mem_flags flags, size_t size)
{
...
size_t HashburstMiner::OclBaseRunner::align(size_t size) const
{
    return VirtualMemory::align(size, m_align);
}


void HashburstMiner::OclBaseRunner::enqueueReadBuffer(cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr)
{
...
void HashburstMiner::OclBaseRunner::enqueueWriteBuffer(cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr)
{
...
void HashburstMiner::OclBaseRunner::finalize(uint32_t *hashOutput)
{
