...
cl_mem HashburstMiner::OclSharedData::createBuffer(cl_context context, size_t size, size_t &offset, size_t limit)
{
...
uint64_t HashburstMiner::OclSharedData::adjustDelay(size_t /*id*/)
{
    if (m_threads < 2) {
        return 0;
    }
...
#   ifdef HashburstMiner_INTERLEAVE_DEBUG
...
uint64_t HashburstMiner::OclSharedData::resumeDelay(size_t /*id*/)
{
    if (m_threads < 2) {
        return 0;
    }
...
void HashburstMiner::OclSharedData::release()
{
...
void HashburstMiner::OclSharedData::setResumeCounter(uint32_t value)
{
    if (m_threads < 2) {
        return;
    }
...
void HashburstMiner::OclSharedData::setRunTime(uint64_t time)
{
...
#ifdef HashburstMiner_ALGO_RANDOMX
cl_mem HashburstMiner::OclSharedData::dataset() const
{
    if (!m_dataset) {
...
void HashburstMiner::OclSharedData::createDataset(cl_context ctx, const Job &job, bool host)
{
    if (m_dataset) {
        return;
    }
