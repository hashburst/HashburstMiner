...
HashburstMiner::OclSharedData &HashburstMiner::OclSharedState::get(uint32_t index)
{
    return map[index];
}


void HashburstMiner::OclSharedState::release()
{
    for (auto &kv : map) {
        kv.second.release();
    }
...
void HashburstMiner::OclSharedState::start(const std::vector<OclLaunchData> &threads, const Job &job)
{
    assert(map.empty());
