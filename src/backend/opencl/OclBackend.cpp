...
namespace HashburstMiner { static const char *kAdlLabel = "ADL"; }
#endif
...
const char *HashburstMiner::ocl_tag()
{
    return Tags::opencl();
}


HashburstMiner::OclBackend::OclBackend(Controller *controller) :
...
HashburstMiner::OclBackend::~OclBackend()
{
    delete d_ptr;
...
bool HashburstMiner::OclBackend::isEnabled() const
{
    return d_ptr->controller->config()->cl().isEnabled() && OclLib::isInitialized() && d_ptr->platform.isValid() && !d_ptr->devices.empty();
}


bool HashburstMiner::OclBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cl().threads().get(algorithm).isEmpty();
}


const HashburstMiner::Hashrate *HashburstMiner::OclBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const HashburstMiner::String &HashburstMiner::OclBackend::profileName() const
{
    return d_ptr->profileName;
}


const HashburstMiner::String &HashburstMiner::OclBackend::type() const
{
    return kType;
}


void HashburstMiner::OclBackend::execCommand(char)
{
}


void HashburstMiner::OclBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void HashburstMiner::OclBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void HashburstMiner::OclBackend::printHealth()
{
#   ifdef HashburstMiner_FEATURE_ADL
    d_ptr->printHealth();
#   endif
...
void HashburstMiner::OclBackend::setJob(const Job &job)
{
...
void HashburstMiner::OclBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void HashburstMiner::OclBackend::stop()
{
...
bool HashburstMiner::OclBackend::tick(uint64_t ticks)
{
...
#ifdef HashburstMiner_FEATURE_API
rapidjson::Value HashburstMiner::OclBackend::toJSON(rapidjson::Document &doc) const
{
...
void HashburstMiner::OclBackend::handleRequest(IApiRequest &)
{
}
#endif
