...
HashburstMiner::CudaWorker::CudaWorker(size_t id, const CudaLaunchData &data) :
...
HashburstMiner::CudaWorker::~CudaWorker()
{
    delete m_runner;
}


void HashburstMiner::CudaWorker::jobEarlyNotification(const Job &job)
{
    if (m_runner) {
...
bool HashburstMiner::CudaWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t HashburstMiner::CudaWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void HashburstMiner::CudaWorker::start()
{
...
bool HashburstMiner::CudaWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::CUDA) == 0) {
        return false;
    }
...
void HashburstMiner::CudaWorker::storeStats()
{
    if (!isReady()) {
        return;
    }
