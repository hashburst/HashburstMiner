...
bool HashburstMiner::NvmlLib::init(const char *fileName)
{
    if (!m_initialized) {
        m_loader      = fileName;
        m_ready       = dlopen() && load();
...
const char *HashburstMiner::NvmlLib::lastError() noexcept
{
    return uv_dlerror(&nvmlLib);
}


void HashburstMiner::NvmlLib::close()
{
    if (m_ready) {
        pNvmlShutdown();
    }
...
bool HashburstMiner::NvmlLib::assign(std::vector<CudaDevice> &devices)
{
    uint32_t count = 0;
...
NvmlHealth HashburstMiner::NvmlLib::health(nvmlDevice_t device)
{
    if (!device) {
        return {};
    }
...
bool HashburstMiner::NvmlLib::dlopen()
{
    if (!m_loader.isNull()) {
...
bool HashburstMiner::NvmlLib::load()
{
    try {
