...
HashburstMiner::CudaThread::CudaThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
HashburstMiner::CudaThread::CudaThread(uint32_t index, nvid_ctx *ctx) :
...
bool HashburstMiner::CudaThread::isEqual(const CudaThread &other) const
{
...
rapidjson::Value HashburstMiner::CudaThread::toJSON(rapidjson::Document &doc) const
{
