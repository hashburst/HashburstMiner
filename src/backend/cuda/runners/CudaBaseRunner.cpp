...
HashburstMiner::CudaBaseRunner::CudaBaseRunner(size_t id, const CudaLaunchData &data) :
    m_data(data),
    m_threadId(id)
{
}
...
HashburstMiner::CudaBaseRunner::~CudaBaseRunner()
{
    CudaLib::release(m_ctx);
}


bool HashburstMiner::CudaBaseRunner::init()
{
...
bool HashburstMiner::CudaBaseRunner::set(const Job &job, uint8_t *blob)
{
    m_height = job.height();
    m_target = job.target();
...
size_t HashburstMiner::CudaBaseRunner::intensity() const
{
    return m_data.thread.threads() * m_data.thread.blocks();
}


bool HashburstMiner::CudaBaseRunner::callWrapper(bool result) const
{
    if (!result) {
