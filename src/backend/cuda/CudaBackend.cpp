...
#ifdef HashburstMiner_ALGO_KAWPOW
...
#ifdef HashburstMiner_FEATURE_NVML
#include "backend/cuda/wrappers/NvmlLib.h"

namespace HashburstMiner { static const char *kNvmlLabel = "NVML"; }
#endif
...
const char *HashburstMiner::cuda_tag()
{
    return Tags::nvidia();
}


HashburstMiner::CudaBackend::CudaBackend(Controller *controller) :
...
HashburstMiner::CudaBackend::~CudaBackend()
{
    delete d_ptr;
...
bool HashburstMiner::CudaBackend::isEnabled() const
{
    return d_ptr->controller->config()->cuda().isEnabled() && CudaLib::isInitialized() && !d_ptr->devices.empty();;
}


bool HashburstMiner::CudaBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cuda().threads().get(algorithm).isEmpty();
}


const HashburstMiner::Hashrate *HashburstMiner::CudaBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const HashburstMiner::String &HashburstMiner::CudaBackend::profileName() const
{
    return d_ptr->profileName;
}


const HashburstMiner::String &HashburstMiner::CudaBackend::type() const
{
    return kType;
}


void HashburstMiner::CudaBackend::execCommand(char)
{
}


void HashburstMiner::CudaBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void HashburstMiner::CudaBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void HashburstMiner::CudaBackend::printHealth()
{
#   ifdef HashburstMiner_FEATURE_NVML
    d_ptr->printHealth();
#   endif
...
void HashburstMiner::CudaBackend::setJob(const Job &job)
{
...
void HashburstMiner::CudaBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void HashburstMiner::CudaBackend::stop()
{
...
bool HashburstMiner::CudaBackend::tick(uint64_t ticks)
{
...
#ifdef HashburstMiner_FEATURE_API
rapidjson::Value HashburstMiner::CudaBackend::toJSON(rapidjson::Document &doc) const
{
...
void HashburstMiner::CudaBackend::handleRequest(IApiRequest &)
{
}
#endif
