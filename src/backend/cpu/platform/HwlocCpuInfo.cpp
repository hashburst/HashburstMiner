 ...
 * Copyright (c) 2016-2023 HashburstMiner       <pegoraro.gabriele@gmail.com>
 *
...
#ifdef HashburstMiner_HWLOC_DEBUG
#   include <uv.h>
#endif
...
HashburstMiner::HwlocCpuInfo::HwlocCpuInfo()
{
...
#   if defined(HashburstMiner_OS_MACOS) && defined(HashburstMiner_ARM)
...
bool HashburstMiner::HwlocCpuInfo::membind(hwloc_const_bitmap_t nodeset)
{
...
HashburstMiner::CpuThreads HashburstMiner::HwlocCpuInfo::threads(const Algorithm &algorithm, uint32_t limit) const
{
#   ifndef HashburstMiner_ARM
    if (L2() == 0 && L3() == 0) {
...
HashburstMiner::CpuThreads HashburstMiner::HwlocCpuInfo::allThreads(const Algorithm &algorithm, uint32_t limit) const
{
    CpuThreads threads;
    threads.reserve(m_threads);
...
void HashburstMiner::HwlocCpuInfo::processTopLevelCache(hwloc_obj_t cache, const Algorithm &algorithm, CpuThreads &threads, size_t limit) const
{
#   ifndef HashburstMiner_ARM
...
void HashburstMiner::HwlocCpuInfo::setThreads(size_t threads)
{
    if (!threads) {
        return;
    }
