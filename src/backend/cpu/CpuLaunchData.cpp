...
HashburstMiner::CpuLaunchData::CpuLaunchData(const Miner *miner, const Algorithm &algorithm, const CpuConfig &config, const CpuThread &thread, size_t threads, const std::vector<int64_t>& affinities) :
    algorithm(algorithm),
...
bool HashburstMiner::CpuLaunchData::isEqual(const CpuLaunchData &other) const
{
...
HashburstMiner::CnHash::AlgoVariant HashburstMiner::CpuLaunchData::av() const
{
    if (intensity <= 2) {
...
const char *HashburstMiner::CpuLaunchData::tag()
{
    return cpu_tag();
}
