...
template<size_t N>
HashburstMiner::CpuWorker<N>::CpuWorker(size_t id, const CpuLaunchData &data) :
...
template<size_t N>
HashburstMiner::CpuWorker<N>::~CpuWorker()
{
#   ifdef HashburstMiner_ALGO_RANDOMX
    RxVm::destroy(m_vm);
#   endif
...
#ifdef HashburstMiner_ALGO_RANDOMX
template<size_t N>
void HashburstMiner::CpuWorker<N>::allocateRandomX_VM()
{
...
template<size_t N>
bool HashburstMiner::CpuWorker<N>::selfTest()
{
#   ifdef HashburstMiner_ALGO_RANDOMX
...
template<size_t N>
void HashburstMiner::CpuWorker<N>::hashrateData(uint64_t &hashCount, uint64_t &, uint64_t &rawHashes) const
{
    hashCount = m_count;
    rawHashes = m_count;
}
...
template<size_t N>
void HashburstMiner::CpuWorker<N>::start()
{
...
template<size_t N>
bool HashburstMiner::CpuWorker<N>::nextRound()
{
#   ifdef HashburstMiner_FEATURE_BENCHMARK
...
template<size_t N>
bool HashburstMiner::CpuWorker<N>::verify(const Algorithm &algorithm, const uint8_t *referenceValue)
{
#   ifdef HashburstMiner_ALGO_GHOSTRIDER
...
template<size_t N>
bool HashburstMiner::CpuWorker<N>::verify2(const Algorithm &algorithm, const uint8_t *referenceValue)
{
    cn_hash_fun func = fn(algorithm);
    if (!func) {
        return false;
...
template<size_t N>
void HashburstMiner::CpuWorker<N>::allocateCnCtx()
{
    if (m_ctx[0] == nullptr) {
        int shift = 0;
...
template<size_t N>
void HashburstMiner::CpuWorker<N>::consumeJob()
{
    if (Nonce::sequence(Nonce::CPU) == 0) {
        return;
    }
