...
#ifdef HashburstMiner_ALGO_ARGON2
...
const char *HashburstMiner::backend_tag(uint32_t backend)
{
...
const char *HashburstMiner::cpu_tag()
{
    return Tags::cpu();
}


HashburstMiner::CpuBackend::CpuBackend(Controller *controller) :
...
HashburstMiner::CpuBackend::~CpuBackend()
{
    delete d_ptr;
}


bool HashburstMiner::CpuBackend::isEnabled() const
{
    return d_ptr->controller->config()->cpu().isEnabled();
}


bool HashburstMiner::CpuBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cpu().threads().get(algorithm).isEmpty();
}


bool HashburstMiner::CpuBackend::tick(uint64_t ticks)
{
    return d_ptr->workers.tick(ticks);
}


const HashburstMiner::Hashrate *HashburstMiner::CpuBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const HashburstMiner::String &HashburstMiner::CpuBackend::profileName() const
{
    return d_ptr->profileName;
}


const HashburstMiner::String &HashburstMiner::CpuBackend::type() const
{
    return kType;
}


void HashburstMiner::CpuBackend::prepare(const Job &nextJob)
{
#   ifdef HashburstMiner_ALGO_ARGON2
...
void HashburstMiner::CpuBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void HashburstMiner::CpuBackend::printHealth()
{
}


void HashburstMiner::CpuBackend::setJob(const Job &job)
{
    if (!isEnabled()) {
        return stop();
    }
...
void HashburstMiner::CpuBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void HashburstMiner::CpuBackend::stop()
{
...
#ifdef HashburstMiner_FEATURE_API
rapidjson::Value HashburstMiner::CpuBackend::toJSON(rapidjson::Document &doc) const
{
...
#   ifdef HashburstMiner_FEATURE_ASM
...
void HashburstMiner::CpuBackend::handleRequest(IApiRequest &request)
{
...
#ifdef HashburstMiner_FEATURE_BENCHMARK
HashburstMiner::Benchmark *HashburstMiner::CpuBackend::benchmark() const
{
    return d_ptr->benchmark.get();
}


void HashburstMiner::CpuBackend::printBenchProgress() const
{
    if (d_ptr->benchmark) {
