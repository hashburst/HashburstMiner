...
HashburstMiner::Msr::Msr() : d_ptr(new MsrPrivate())
{
    if (!isAvailable()) {
...
HashburstMiner::Msr::~Msr()
{
    delete d_ptr;
}


bool HashburstMiner::Msr::isAvailable() const
{
    return d_ptr->isAvailable();
}


bool HashburstMiner::Msr::write(Callback &&callback)
{
...
bool HashburstMiner::Msr::rdmsr(uint32_t reg, int32_t cpu, uint64_t &value) const
{
...
bool HashburstMiner::Msr::wrmsr(uint32_t reg, uint64_t value, int32_t cpu)
{
